...
**① 确认输入类型和数量：**
Creator 回答后，先确认输入类型（ASIN/关键词/文件等），再按下面的属性判断决定是否追问数量：

- 如果 Creator 的回答只是照抄了示例词（如只回"关键词"）或很含糊，确认类型时要结合 `.minus/skill.json` 中已有的 name/description 把它放回语境复述（如该 Skill 名称是「相关关键词推荐」，复述"也就是用户输入一个关键词，你的 Skill 会围绕它推荐相关关键词，对吧？"），请 Creator 确认后再继续——没想清楚的回答经不起具体化，这里能拦住。
  ⛔ 复述内容必须来自 skill.json 已有的名称/描述，禁止仅凭输入类型本身臆测业务场景（如看到"关键词"就默认是市场调研/选品分析）——这和纵容 Creator 照抄示例词是同一种错误，只是换了个地方发生。
...
（假设 `.minus/skill.json` 中该 Skill 的 name 是「相关关键词推荐」）
...
Plugin: 也就是用户输入一个关键词，你的 Skill 会围绕它推荐相关关键词，对吧？  ← 回答与示例词雷同，结合 skill.json 已有名称放回语境复述确认，不臆测业务场景

Creator: 对

Plugin: 好的。用户可以输入几个？只能一个、一个或多个、还是至少两个  ← 数量说得通且未明确，原样追问
