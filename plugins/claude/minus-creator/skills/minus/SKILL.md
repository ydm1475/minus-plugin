...
Plugin 引导 Creator 按顺序开发每个 pipeline 节点。**严格按四个维度逐步推进，每个维度必须问 Creator 确认后才能进入下一个。**
...
- 通过数据服务商 MCP 自动发现可用 API，向 Creator 推荐匹配的接口
- Creator 不需要自己查 API 文档

对话示例：
```
Plugin: 现在开发步骤 1「关键词数据采集」。
