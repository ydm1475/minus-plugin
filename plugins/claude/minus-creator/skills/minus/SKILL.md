...
```
Plugin: 最后一个问题：Skill 跑完之后，最终给用户看什么结果？
       比如一份报告、一个关键词列表、一个评分……
...
Plugin 引导 Creator 按顺序开发每个 pipeline 节点。严格按四个维度逐步推进，和三步法一样的规则：**每个维度必须问 Creator、等回答、确认后才能进入下一个。**

⛔ 核心规则：每次回复最多推进一个维度。代码写完后，你的回复必须以下一个维度的问题结尾。
⛔ 禁止：在一次回复中完成多个维度的代码
⛔ 禁止：跳过任何维度（即使觉得答案显而易见）
⛔ 禁止：四个维度全部确认之前就让 Creator"试一下效果"
...
通过 MCP 自动发现可用 API，向 Creator 推荐匹配的接口。Creator 确认后编写数据获取代码。
代码写完后，回复末尾必须原样输出：
...
Creator 确认后编写输出渲染代码。
代码写完后，回复末尾必须原样输出：
「输出已写好。最后一个问题：用户运行到这一步后，需要暂停确认再继续吗？还是自动往下走？」

### ④ 用户确认 → 标记节点完成

Creator 确认后，标记该节点四维度全部完成，进入下一个节点。

**代码编写、调试与测试贯穿节点开发全程。** Plugin 在每个维度确认后即时编写和调试代码，Creator 随时可在浏览器中体验效果并提出修改意见。
