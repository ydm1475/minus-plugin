...
引导 Creator 逐一确认以下两个方面：

### ① 结果摘要：如何总结这个 Skill 的运行结果？

先展示各步骤产出的数据全景，再引导 Creator 定义摘要的风格和内容。

```
Plugin: 所有步骤开发完成。各步骤产出的数据：
       · 步骤 1：关键词基础数据（搜索量、趋势）
       · 步骤 2：竞争度评分和分析报告
       · 步骤 3：推荐长尾词列表
       ...
       你想怎么定义这段摘要？比如：
       "关键词 wireless earbuds 月搜索量 12 万，竞争度中等，建议优先布局以下 3 个长尾词……"

Creator: 差不多这个感觉，大模型自动生成就行，但要突出关键数字和结论
```

### ② 下载内容：提供哪些可下载的文件？

```
Plugin: 用户可以下载哪些内容？默认支持两种格式：
       · Excel：推荐词列表（关键词 + 搜索量 + 竞争度 + 潜力评分）
       · HTML 报告：完整分析报告，可在浏览器中打开查看
       你觉得这两个够吗？还需要其他格式？
