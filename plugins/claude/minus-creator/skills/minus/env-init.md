...
读 `CLAUDE_CODE_ENTRYPOINT` 环境变量：claude-desktop/vscode/jetbrains 为 Desktop，其余为 CLI。
...
```bash
minus-lib resume-env <desktop|cli>
```
调用时给 `Bash` 传 `timeout: 120000`。按输出处理：
...
- `ok` → 重跑 `minus-lib resume-env <分支>` 继续。
