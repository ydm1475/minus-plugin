...
「我的建议：[结合当前步骤的名称/目的和维度 ① 已确认的具体数据，给出贴合当前步骤的建议。判断这一步的产出是结构化数据（排序、筛选、聚合）、自然语言内容（分析报告、推荐理由、趋势解读），还是两者都有，分别建议对应的处理方式]」
...
回顾 Creator **在当前步骤**维度 ①② 中的回答，判断 Creator 是否已经说了要展示什么（⛔ 其他步骤的对话不算）：
