...
- **区块内追问带锚点**：同一维度内需要多轮追问时，追问开头带上区块名（区块名从维度清单取），让 Creator 知道这是同一话题的深入，不是新话题。单轮问答不需要。
...
本清单是维度的唯一权威定义。开场白、区块提示、维度切换话术都从这里取名字，禁止在别处写死维度名称或数量。
...
进入四维度流程前，先读 `.minus/total-steps`，将当前步骤编号与该数字比较，判断是否为最后一步（结果在后续维度中复用，不重复读取）。

从维度清单取适用于当前步骤的区块（最后一步排除④），动态拼出开场白：

「开发第 N 步[步骤名]（共 M 步{最后一步时追加"，这是最后一步"}）。我们要确认{件数}件事：{按序号罗列适用区块名}——先看{第一个区块名}。」

⛔ 件数和名称必须从清单动态拼出，禁止不看清单就写死件数。
⛔ 数字风格：{件数}用汉字（如"三件事""四件事"），与结尾"先看第一件"一致；步骤编号 N 和 M 保持阿拉伯数字（如"第 1 步（共 2 步）"）。同一句话里不得对同类数字混用两种写法。
⛔ "第 N 步"和"[步骤名]"之间不能插入其他信息（如"共 M 步"）——编号和名字是同一件事的两面，不能被打断。M（共几步）和"这是最后一步"合并进步骤名后的同一个括号，用逗号分隔，不要写成两层嵌套括号。M 复用上面已读取的 `.minus/total-steps`，不重复读文件。

**开场白每步只说一次**，修改场景（见「修改已完成步骤的处理逻辑」）不说。
...
- **llm**：LLM 的分析产出自动作为步骤摘要（summary），不需要 Creator 说"我要摘要"。回顾 Creator **在当前步骤**维度 ①② 中的回答，判断是否已经提到了除摘要之外的额外展示内容（如图表、表格、曲线等）：

  - **已提到额外展示内容**（Creator 在维度 ② 讨论中确认了具体展示形式，无论是 Creator 主动说的还是 Agent 建议后 Creator 确认的）→ **禁止再问"还需要额外展示什么"**，改为复述确认：「大模型的分析内容会自动展示为步骤摘要，加上你前面确认的 [复述具体内容，如"趋势折线图"]。展示内容就按这个来，可以吗？」Creator 确认后 → 进入下一维度
  - **未提到额外展示内容**（维度 ② 只确认了处理逻辑，没涉及具体展示形式）→ 向 Creator 输出（**同一条消息内全部说完**）：
...
**最后一步判断**：沿用进场开场白时已得出的判断结果（进场时已读 `.minus/total-steps` 并比较，此处不重复读取）。最后一步没有维度 ④（最后一步没有"传给下一步"的概念），直接进入阶段二。
...
然后列出推荐，**每条数据单独一行（列表条目），禁止用"；"串在同一段里**，说明每条数据后面哪些步骤会用到。interactive 模式下注意区分哪些是用户选择的数据（如勾选的关键词）、哪些是自动带过去的（如类目、站点信息）。例如这一步展示了关键词表格，就输出：

```
- **用户选中的关键词** — 用户从表格里勾选的那些词
- **类目身份**（哪个类目、哪个站点） — 后面每步都要用，自动带过去不需要用户操心
...
然后列出推荐，**每条数据单独一行（列表条目），禁止用"；"串在同一段里**，说明每条数据后面哪些步骤会用到。例如这一步做了关键词筛选，就输出：

```
- **核心关键词清单** — 这一步已经查出来了，后面分析直接用
- **类目身份**（哪个类目、哪个站点） — 后面分析要围绕它
...
`generate-node-code` 的输出里有 `FRONTEND_DOC_URLS=N` 和 N 条现成的 curl 命令——这是本步骤的必读子文档清单（对齐后端 `SDK_DOC_PATH` 的处理方式）。逐条检查：该文档本会话已 curl 过、且**全文仍在当前上下文中**（没有被压缩成摘要）→ 可跳过；只记得结论、原文已不在上文 → 重新 curl。判断标准是"原文在不在"，不是"我记不记得"——拿不准就重新 curl。`FRONTEND_DOC_URLS=0` 表示脚本没能解析出文档地址，此时按项目 CLAUDE.md 的「前端 SDK 参考」章节取文档。注意：部分文档地址是**索引**而非文档本身，必须按索引读取对应的子文档，只读索引不读子文档 = 没读文档。

文档不可达时，明确告诉 Creator 并停止写前端代码。⛔ 禁止凭记忆写 props 或假设框架行为。

#### 写前端代码前，必须完成：

1. **必读子文档**：`generate-node-code` 输出的 `FRONTEND_DOC_URLS` 清单逐条过一遍——全文仍在上下文的可跳过，其余执行 curl 并通读。
2. **组件选型**：读 CLAUDE.md「前端 SDK 参考」中的组件索引（已读过则跳过），对照维度 ③ 确认的展示需求选出合适的现成组件。**只有确认没有合适组件时才手写 JSX。**
3. **确认 props**：选定组件后，从文档中确认 props、回调签名等。组件选择、prop 名称、回调签名都以文档为准——凭记忆写大概率对不上当前版本。
...
- Creator 在任意维度可以修改前面已确认的内容，不视为异常：更新对应维度的记录，如果改动影响到已确认的后续维度，简要说明受影响部分并只重新确认那部分（与「修改已完成步骤」的受影响维度判定表同一套逻辑），然后从当前进度继续。
