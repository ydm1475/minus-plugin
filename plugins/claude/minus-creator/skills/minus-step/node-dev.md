...
处理逻辑可以是确定性代码、SDK LLM 能力，或两者组合：

- 格式化、排序、过滤、聚合 → 纯代码
- 分析摘要、趋势解读、推荐理由、风险提示、文案生成 → 可使用 SDK 内置 LLM 能力
- 一步内可以同时包含两种：先用代码做结构化处理（排序/筛选/聚合），再用大模型基于处理后的数据生成分析内容

原则：能用确定性代码解决的部分不用 LLM；需要自然语言理解/判断/生成的部分用 LLM。两者不互斥。

建议判断原则：**建议要服务于步骤的目的，不能只匹配数据类型。** 步骤名称传达的期望是得出结论/判断/解读时，即使数据本身是结构化的，也应该建议用 LLM。
- 正例：步骤叫「热门产品分析」，数据是价格/销量/评分 → 建议 LLM（Creator 要的是"分析"，不是排序表）
- 反例：步骤叫「筛选高评分产品」，数据同样是价格/销量/评分 → 建议纯确定性（Creator 要的是筛选结果，不是分析文章）
...
**首先判断：维度 ② 是否包含 LLM 分析/总结/解读？**

- **是（② 包含 LLM）**：LLM 的分析产出自动作为步骤摘要（summary），不需要 Creator 说"我要摘要"。向 Creator 输出（**同一条消息内全部说完**）：
...
- **否（② 是纯确定性处理）**：回顾 Creator **在当前步骤**维度 ①② 中的回答，判断是否已经说了要展示什么（⛔ 其他步骤的对话不算）：
