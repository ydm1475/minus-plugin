...
# 1.5 Node 版本 gate（推荐/下限版本单源 toolchain.sh）
...
# 下限单源于 toolchain.sh（NODE_RUNTIME_FLOOR），勿在此写死版本号
...
# 非交互：用户多为非程序员，无法回答"要不要装 Node"；Volta 装在用户目录、
# 可逆无破坏，缺/旧直接自动安装，失败才报错（原则 #6：工程化，不靠用户决策）。
echo ""
echo "→ 检查 Node 版本（建议 Node ${NODE_TARGET}）..."
NMAJ="$(node_major)"
if [ -z "$NMAJ" ] || [ "$NMAJ" -lt "$NODE_RUNTIME_FLOOR" ] 2>/dev/null; then
  if [ -z "$NMAJ" ]; then
    echo "  未检测到 Node.js，自动安装 Node ${NODE_TARGET}（Volta，用户目录，可逆）..."
  else
    echo "  当前 Node $(node -v 2>/dev/null) 过旧（最低 ${NODE_RUNTIME_FLOOR}），自动升级到 Node ${NODE_TARGET}..."
  fi
  if provision_node_via_volta; then
    echo -e "${GREEN}✓${NC} Node 已就绪（$(node -v 2>/dev/null)，Volta 管理 Node ${NODE_TARGET}）"
  else
    echo "❌ Node ${NODE_TARGET} 自动安装失败。请手动安装 Node ${NODE_TARGET}（推荐 https://volta.sh）后重跑 install.sh。"
    exit 1
  fi
elif [ "$NMAJ" -lt "$NODE_FLOOR" ] 2>/dev/null; then
  echo -e "${GREEN}✓${NC} Node $(node -v 2>/dev/null) 可用；建议升级到 Node ${NODE_TARGET} 以获得最佳体验。"
else
