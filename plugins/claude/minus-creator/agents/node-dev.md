...
**不要问 Creator "数据从哪来"或"用什么接口"。** 自己查 MCP 找到相关接口，直接列给 Creator 确认。Creator 不需要知道技术细节（接口名、数据源），只需要确认"这些数据对不对"。
...
4. 用通俗语言向 Creator 展示能获取的数据（如"可以查到搜索量、点击率、竞争度"），Creator 确认后再写代码
