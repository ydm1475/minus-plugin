...
if [ -f "$MINUS_JSON" ]; then
  ...
  echo "4. 执行 Bash(npm run dev) 启动开发服务器（后台运行），不要等它完成。"
  echo "   启动后告知 Creator 预览地址：http://localhost:{port}（不要加 /preview）"
  ...
  echo "      「接下来建议先聊聊这个 Skill 要做什么，我来帮你设计步骤结构。」"
  echo "   禁止：读取或分析项目代码、描述页面组件（输入框、按钮等）"
  echo "6. 如果不是首次进入，报告状态：✓ Minus 已就绪 — {Skill名称} v{版本}"
  echo "7. 如果有未完成进度，告知上次做到哪里、下一步建议做什么。"
  echo "8. 如果无进度且非首次，引导 Creator 继续开发或选择其他操作。"
  echo "用通俗语言，不要技术术语。"
  echo "</context>"
