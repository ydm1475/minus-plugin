...
if [ -f "$MINUS_JSON" ]; then
  ...
  echo "4. 启动开发服务器前，先清理可能残留的旧进程："
  echo "   执行 Bash(pkill -f 'uvicorn server:app' 2>/dev/null; pkill -f 'npx vite' 2>/dev/null; pkill -f 'concurrently' 2>/dev/null; sleep 1)"
  echo "   然后执行 Bash(npm run dev) 启动开发服务器（后台运行）。"
  ...
  echo "   必须用 Vite 输出的前端端口（通常 5173 开始）。"
  echo "   如果客户端是 Desktop 版：只输出预览地址 http://localhost:{前端端口}，Desktop 会自动弹出预览面板，不要执行 open 命令。"
  echo "   如果客户端是 CLI 版：执行 Bash(open http://localhost:{前端端口}) 打开浏览器。"
  ...
  echo "      「最后一个问题：所有步骤都跑完之后，最终给用户看什么结果？」"
  echo "      「比如一份报告、一个关键词列表、一个评分……」"
  echo "      注意：这里问的是最终汇总结果，不是每一步各自的输出（每步输出在逐节点开发时再定）"
  ...
  echo "   e. 逐节点开发时，按四个维度推进："
  echo "      核心规则：Creator 一句话覆盖多个维度时可合并推进，没覆盖的维度要问。"
  echo "      ① 数据需求：不要问 Creator「数据从哪来」。先查 .mcp.json 中的数据服务商 MCP，自动发现相关 API，直接列给 Creator 确认。"
  echo "      ② 处理逻辑：Creator 确认后写代码"
  echo "      ③ 输出定义：Creator 确认后写展示代码"
  echo "      ④ 用户确认：如果当前是最后一步（pipeline 最后一个 step），直接跳过本维度，不要问「暂停确认还是自动往下走」"
  echo "      ⛔ 禁止：Creator 没说过的意图不能替 Creator 决定"
