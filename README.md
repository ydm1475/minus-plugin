...
bash $LIB/update-progress.sh <init-design|design-done|append-steps|step-done|confirm-test|set-phase|touch|show>
                                           # progress.json 唯一写入入口（Skill 项目目录下）
                                           # step-done 输出测试邀请话术；confirm-test 记录 Creator 确认整体测试通过
...
bash $SKL/check-running-flow.sh            # 探测是否有进行中的流程执行（RUNNING/IDLE）；
                                           # 改 pipeline.py / 重启 dev server 前必须先跑，避免打断 Creator 测试

STP=~/minus-platform-develop/minus-plugin/plugins/claude/minus-creator/skills/minus-step/scripts
bash $STP/check-frontend.sh                # 前端 tsc 类型检查硬门禁（FRONTEND_OK / GATE_FAILED）
bash $STP/locale-set.sh set <file> <key> <value>  # locale JSON 安全写入（禁止手工 Edit locale 文件）
bash $STP/locale-set.sh rm <file> <key>           # locale JSON 安全删除
```
