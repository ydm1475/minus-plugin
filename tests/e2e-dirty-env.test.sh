...
wait_port_up() {
  ...
  if [ $W -ge 30 ]; then
    echo "  [warn] wait_port_up: port $1 未在 30s 内就绪" >&2
    return 1
  fi
}
...
if [ "$HAS_PY" = "1" ] && [ "$HAS_LSOF" = "1" ]; then
  ...
  if wait_port_up 5191; then
    ...
    skip "非归属 server 门禁拒绝" "python3 未在 30s 内绑定端口"
  fi
else
...
if [ "$HAS_PY" = "1" ]; then
  ...
  if ! wait_port_up 5192; then
    skip "归属旧 server 复用/半死拦截" "python3 未在 30s 内绑定端口"
  else
