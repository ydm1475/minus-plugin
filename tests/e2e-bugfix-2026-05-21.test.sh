...
if assert_file_contains "$NODE_DEV" "禁止手写 HTML"; then
  pass "node-dev.md 包含禁止手写 HTML table 规范"
else
  fail "node-dev.md 应包含禁止手写 HTML table 规范" "检查维度③编码规范"
fi

if assert_file_contains "$NODE_DEV" "SelectableTableWidget"; then
  pass "node-dev.md 指定使用 SelectableTableWidget"
else
  fail "node-dev.md 应指定使用 SelectableTableWidget" "所有表格统一用框架组件"
fi
...
if assert_file_contains "$NODE_DEV" "默认.*弹框"; then
  pass "node-dev.md 说明 SDK 默认 interactive widget 使用弹框"
else
  fail "node-dev.md 应说明 SDK 默认弹框" "检查前端代码模板部分"
fi

if assert_file_contains "$NODE_DEV" "modal: false.*inline"; then
  pass "node-dev.md 说明写 modal: false 才是 inline"
else
  fail "应说明 modal: false 才是 inline" "检查前端代码模板部分"
fi
