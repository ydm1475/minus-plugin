...
**阶段一：四维度收集意图（不写代码）**，每个维度完成用 `minus-lib step-tracker complete {N} <dim>` 标记，脚本自动输出下一维度话术（硬编码、原样转达）：

- ① 数据需求：Read main.tsx 确认输入类型 → ToolSearch 发现 MCP → `search_api_docs` / `get_endpoint_details` 查接口 → 通俗语言向 Creator 展示
- ② 处理逻辑：确定性代码（格式化/排序/聚合）vs LLM（分析/摘要/推荐）；标记带模式 `logic llm|deterministic`
- ③ 输出定义：只收集展示意图，禁止自动补展示；最后一步时脚本自动跳过维度 ④，输出 `NEXT=GENERATE`
- ④ 用户确认 + 传递数据（仅非最后一步）：confirm 模式 auto|interactive + 传给下一步的数据

**阶段二：一次性生成代码**——门禁 `minus-lib generate-node-code {N}` 输出 GATE_PASSED 才能写；输出 LOGIC_MODE / CONFIRM_MODE / IS_LAST 供生成参考。
...
所有节点开发完成后执行 `minus-lib generate-result-design`：门禁（全步骤四维度完成）→ 从 pipeline.py 提取 payload 全景 → 输出两维度引导（结果摘要 + 下载内容）。
...
- **progress-check.sh**：挂 SessionStart/Stop hook，从硬产物（pipeline.py 步骤数、dev-progress 标记）单向收敛 progress.json，兜底 Agent 漏调
- **step-tracker.sh**：四维度标记存 `.minus/dev-progress/`（step_N_data / logic / logic_mode / output / confirm / confirm_mode）
...
- `.minus/dev-progress/`：`step_N_data/logic/output/confirm` 空文件（存在即完成）；`step_N_logic_mode`（deterministic|llm）、`step_N_confirm_mode`（auto|interactive）存模式值
...
- 自带两道硬门禁：step-tracker check 必须 COMPLETE + step_N 函数体无 `# TODO`，否则 exit 1

**自愈器** progress-check.sh 挂 SessionStart + Stop hook（hooks/hooks.json），单向收敛只升不降：硬产物完成但 json 没标 → 补标；json 丢失 → 重建（testing/ready 的 phase 保留不降级）；currentStep 重算；全完成且 developing → 推 testing。
...
→ minus skill → resume-env.sh 输出：
    ...
    STEP_STATUS=INCOMPLETE: 处理逻辑 用户确认    ← step-tracker check
→ dev-phase.md 按字段路由（不自己读 json）→ node-dev.md
→ Agent 从 STEP_STATUS 知道步骤 2 缺维度②④，从维度②续问
```

恢复粒度精确到"某步骤的某维度"，全部从文件系统反推，用户无需复述进度。
