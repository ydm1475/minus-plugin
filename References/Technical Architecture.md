...
| `SessionStart` hook | Hook | 打开项目文件夹时自动检测 |
...
# 检查当前目录是否是 Minus Skill 项目

if [ -f ".minus.json" ]; then
  echo "<context>
  当前目录是 Minus Skill 项目。
  项目信息：$(cat .minus.json)
  
  如果用户打招呼或开始对话，主动告知项目状态。
  如果有未完成的进度，提醒用户。
  </context>"
fi
```

SessionStart hook 的输出会注入到 Claude Code 的上下文中，Plugin 在对话开始时就知道当前目录的状态。
...
| Tool | 用途 | 对应 Phase |
|------|------|-----------|
| `auth_login` | Creator 登录 | Phase 2 |
| `auth_refresh` | 刷新凭证 | Phase 3 |
| `auth_status` | 检查登录状态 | Phase 3 |
| `skill_create` | 在平台注册新 Skill | Phase 2 |
| `skill_validate` | 发布前校验 | Phase 5.1 |
| `skill_publish` | 上传并发布 | Phase 5.2~5.3 |
| `skill_versions` | 查询已发布版本列表 | Phase 5.1（版本号校验） |
